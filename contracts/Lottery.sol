...
contract LotteryContract {
    ...
    uint256 public constant PLATFORM_FEE = 0.5 ether;  // Fixed platform fee
    uint256 public constant FIRST_PRIZE = 2.5 ether;     // Fixed first prize
    uint256 public constant SECOND_PRIZE = 1.5 ether;  // Fixed second prize
    uint256 public constant THIRD_PRIZE = 0.5 ether;   // Fixed third prize
    ...
    function buyTickets(uint256 _numberOfTickets) external payable roundActive {
        ...
        if (currentRound.totalTickets == maxTickets) {
            ...
            selectAndAwardWinners();
        }
    }
    
    function selectAndAwardWinners() private {
        Round storage currentRound = rounds[currentRoundId];
